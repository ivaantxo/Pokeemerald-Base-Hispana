...
static void PrintMonOTID(void)
{
    ...
    if (InBattleFactory() != TRUE && InSlateportBattleTent() != TRUE)
    {
        ConvertIntToDecimalStringN(StringCopy(gStringVar1, gText_ID), (u16)sMonSummaryScreen->summary.OTID, STR_CONV_MODE_LEADING_ZEROS, 5);
...
{
    int x;
    StringCopy(gStringVar1, gText_ID);
    ...
    if (move != MOVE_NONE)
    {
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
        {
            ...
            if (moveEffect != EFFECT_PLACEHOLDER)
                PrintTextOnWindowToFit(windowId, gMovesInfo[move].description, 6, 1, 0, 0);
            else
...
static u8 LoadMonGfxAndSprite(s16 *state)
{
    ...
    case 0:
        ...
        return 0xFF;
    case 1:
        if (summary->isEgg && PBH_HUEVOS_COLOR_TIPO)
        {
