...
static void DebugAction_Give_Item(u8 taskId)
{
    ...
    AddTextPrinterParameterized(windowId, DEBUG_MENU_FONT, gStringVar4, 0, 0, 0, NULL);
    AddTextPrinterParameterized(windowItemDescription, GetFontIdToFit(ItemId_GetDescription(1), DEBUG_MENU_FONT, 0, WindowWidthPx(windowItemDescription)), ItemId_GetDescription(1), 0, 0, 0, NULL); // Cómo mostrar una ventana con su texto en pantalla:
                                                                                                                                                                                                     // Una vez tenemos la ventana creada, le vamos a printear un texto, en este caso la descripción del item.
                                                                                                                                                                                                     // Al usar el 1 como argumento de ItemId_GetDescription(), le estamos diciendo que cargue la descripción del item nº1, que es la poké ball
                                                                                                                                                                                                     // para después actualizarlo al que sea.
...
static void DebugAction_Give_Item_SelectId(u8 taskId)
{
    if (JOY_NEW(DPAD_ANY))
    {
        ...
        AddTextPrinterParameterized(gTasks[taskId].tWindowId, GetFontIdToFit(ItemId_GetDescription(gTasks[taskId].tInput), DEBUG_MENU_FONT, 0, WindowWidthPx(gTasks[taskId].tWindowId)), ItemId_GetDescription(gTasks[taskId].tInput), 0, 0, 0, NULL); //Cómo mostrar una ventana con su texto en pantalla: Aquí actualizamos la descripción del item según el id correspondiente
