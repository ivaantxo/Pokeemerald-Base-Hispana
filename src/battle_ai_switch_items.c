...
{
    if (gAiLogicData->aiSwitchPredictionInProgress)
...
static bool32 ShouldSwitchIfHasBadOdds(u32 battler)
{
            ...
            && gBattleMons[battler].hp >= gBattleMons[battler].maxHP / 4)))
    {
        ...
        if (RandomPercentage(RNG_AI_SWITCH_HASBADODDS, (100 - GetSwitchChance(SHOULD_SWITCH_HASBADODDS))) && !gAiLogicData->aiSwitchPredictionInProgress)
            return FALSE;
...
static bool32 FindMonThatAbsorbsOpponentsMove(u32 battler)
{
    ...
    if (gBattleStruct->prevTurnSpecies[battler] != gBattleMons[battler].species) // AI mon has changed, player's behaviour no longer reliable; note to override this if using AI_FLAG_PREDICT_MOVE
        return FALSE;
    if (HasSuperEffectiveMoveAgainstOpponents(battler, TRUE) && (RandomPercentage(RNG_AI_SWITCH_ABSORBING_STAY_IN, STAY_IN_ABSORBING_PERCENTAGE) || gAiLogicData->aiSwitchPredictionInProgress))
        return FALSE;
    ...
    for (i = firstId; i < lastId; i++)
    {
        ...
        if (moveFlags & flags)
        {
            ...
            for (j = 0; j < MAX_MON_MOVES; j++)
            {
                ...
                if (AI_GetMoveEffectiveness(move, battler, battlerIn1) >= UQ_4_12(2.0) && (RandomPercentage(RNG_AI_SWITCH_SE_DEFENSIVE, percentChance) || gAiLogicData->aiSwitchPredictionInProgress))
                    return SetSwitchinAndSwitch(battler, i);
            }
        }
    }
...
static bool32 ShouldSwitchIfEncored(u32 battler)
{
    ...
    else if ((RandomPercentage(RNG_AI_SWITCH_ENCORE, GetSwitchChance(SHOULD_SWITCH_ENCORE_DAMAGE)) || gAiLogicData->aiSwitchPredictionInProgress) && gAiLogicData->mostSuitableMonId[battler] != PARTY_SIZE)
        return SetSwitchinAndSwitch(battler, PARTY_SIZE);
...
static bool32 ShouldSwitchIfAttackingStatsLowered(u32 battler)
{
    ...
    if (gBattleMons[battler].attack > gBattleMons[battler].spAttack)
    {
        ...
        {
            if (gAiLogicData->mostSuitableMonId[battler] != PARTY_SIZE && (RandomPercentage(RNG_AI_SWITCH_STATS_LOWERED, GetSwitchChance(SHOULD_SWITCH_ATTACKING_STAT_MINUS_TWO)) || gAiLogicData->aiSwitchPredictionInProgress))
                return SetSwitchinAndSwitch(battler, PARTY_SIZE);
        }
